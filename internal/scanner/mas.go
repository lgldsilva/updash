...
func (s *MASource) Scan(ctx context.Context, plat model.PlatformInfo) ([]*model.Item, error) {
	...
	out, err := execCommand(ctx, binMas, "outdated")
	if err != nil {
		return []*model.Item{
			{Name: binMas, Category: model.CatMAS, Status: model.StatusError, CurrentVer: "error"},
		}, nil
	}
	...
	if len(lines) == 0 || (len(lines) == 1 && lines[0] == "") {
		return []*model.Item{
			{Name: binMas, Category: model.CatMAS, Status: model.StatusOK, CurrentVer: statusUpToDate},
		}, nil
	}
